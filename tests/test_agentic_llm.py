...
from solarchain_eval.agent.llm_client import MockLLMClient
...
def test_mock_llm_client_returns_structured_outputs():
...
def test_llm_fallbacks_on_structured_output_errors():
    class FailingClient:
        mock_used = False
    ...
    audit = auditor.audit(step_context)

    assert plan.governance_mode == "safe_default"
    assert audit.decision == "approve"
    assert planner.failure_count == 1
    assert auditor.failure_count == 1
